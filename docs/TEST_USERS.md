...
curl -X POST http://localhost:8006/api/v1/onboarding/auth/login \
  ...
  -d '{"email": "superadmin@test.rawdrive.ai", "password": "Test@123"}'
...
| Staging | `https://staging.app.RawDrive.io/login` |

### Common Test Scenarios

| Scenario | User | Expected Behavior |
|----------|------|-------------------|
| Free tier limits | `free@test.rawdrive.ai` | Blocked after 3 galleries |
| Admin dashboard | `superadmin@test.rawdrive.ai` | Full platform access |
| Workspace management | `workspaceowner@test.rawdrive.ai` | Can invite/remove members |
| Read-only access | `clientviewer@test.rawdrive.ai` | Cannot create/edit content |
| OAuth flow | `professional@test.rawdrive.ai` | Google OAuth linked |