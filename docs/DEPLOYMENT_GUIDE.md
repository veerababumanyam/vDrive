# RawDrive Complete Deployment Guide
...
# Verify migrations
docker compose exec postgres psql -U RawDrive -d RawDrive -c "SELECT * FROM alembic_version;"
```
...
# Direct psql
psql postgresql://RawDrive:RawDrive@localhost:5432/RawDrive
```
...
-- Database size
SELECT pg_size_pretty(pg_database_size('RawDrive'));
```
...
docker compose exec loki logcli query '{job="RawDrive"}'

# Via API
curl -G -s "http://localhost:3100/loki/api/v1/query" \
  --data-urlencode 'query={job="RawDrive"}' \
...
docker compose exec postgres psql -U RawDrive -d RawDrive -c "SELECT 1;"
...
2. Check health: `docker inspect RawDrive-[service-name] | grep Health`
...
docker compose exec postgres pg_dump -U RawDrive RawDrive > backup_$(date +%Y%m%d).sql

# Schema only
docker compose exec postgres pg_dump -U RawDrive RawDrive --schema-only > schema_$(date +%Y%m%d).sql

# Specific table
docker compose exec postgres pg_dump -U RawDrive RawDrive -t app.users > users_backup.sql
```
...
```bash
# From SQL file
docker compose exec -T postgres psql -U RawDrive RawDrive < backup_20260114.sql

# From compressed backup
gunzip -c backup.sql.gz | docker compose exec -T postgres psql -U RawDrive RawDrive
```
...
SELECT * FROM pg_stat_database WHERE datname = 'RawDrive';
