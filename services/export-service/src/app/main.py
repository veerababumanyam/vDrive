"""
vDrive Export Service - FastAPI Application

Bulk export and migration tools for photographers to download all their photos,
galleries, and metadata. Includes migration wizard for importing from competitor
platforms (Pixieset, Pic-Time, ShootProof, Zenfolio, SmugMug).

Running on port 8009 with PostgreSQL, Redis, Celery, and Cloudflare R2 integration.
"""
...
async def lifespan(app: FastAPI) -> AsyncGenerator:
    ...
    Handles startup and shutdown events for database, Redis, and Celery.
    """
    # Startup
    logger.info(
...
app = FastAPI(
    title="vDrive Export Service",
    description="Bulk export and migration tools for photographers",
    version=settings.SERVICE_VERSION,
...
# Include API router
app.include_router(api_router, prefix="/api/v1/export")
