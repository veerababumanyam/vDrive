"""
FastAPI application for AI Search Service.

Provides semantic search, RAG chat, and caption generation
with Kafka consumers for async embedding generation.
"""
...
async def _validate_dependencies() -> dict[str, str]:
    ...
    if settings.gemini_enabled:
        try:
            ...
            results["gemini"] = "ok" if model else "not_initialized"
        except Exception as e:
...
async def lifespan(app: FastAPI) -> AsyncGenerator:
    ...
    # Start Kafka consumers (will be added in US2)
    # from .consumers.asset_embedding_processor import AssetEmbeddingProcessor
    # embedding_consumer = AssetEmbeddingProcessor()
    # _consumer_tasks.append(asyncio.create_task(embedding_consumer.run()))
    ...
    logger.info("AI Search Service shutdown complete")


# Create FastAPI application
app = FastAPI(
    title="RawDrive AI Search Service",
    description="Semantic search, RAG chat, and caption generation API",
    version=settings.SERVICE_VERSION,
...
async def ready():
    ...
    # Check Gemini (non-critical for search)
    checks["gemini"] = "enabled" if settings.gemini_enabled else "disabled"
